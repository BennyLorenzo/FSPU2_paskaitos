# 1
class Knyga:
    kalba = 'lietuvių k.' # klasės laukas
    ...
    def __str__(self):
        return f"{self.pavadinimas}, {self.autorius}, {self.kalba}, perskaityta {self.perskaityti_puslapiai} iš {self.puslapiu_skaicius} puslapių."
...
class Calculator:
    ...
    def __init__(self, skaicius1: float, skaicius2: float):
...
class Country:
    ...
    def compare_pd(self, other_country):
        ...
        elif self.population/self.area_km2 == other_country.population/other_country.area_km2:
            print(f"{self.name} has the same population density than {other_country.name}.")
        else:
